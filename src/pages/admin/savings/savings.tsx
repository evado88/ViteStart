...
const AdminSavings = () => {
  ...
  const pageConfig = new PageConfig(
    "Administration - Approved Savings",
    `transactions/type/${Assist.TRANSACTION_SAVINGS}/status/${Assist.STATUS_APPROVED}`,
    "",
    "Saving",
    "",
    [Assist.ROLE_ADMIN],
  ...
  const loadData = (year: number) => {
    ...
    const url = `transactions/type/${Assist.TRANSACTION_SAVINGS}/status/${Assist.STATUS_APPROVED}/year/${year}`;
    ...
    setTimeout(() => {
      Assist.loadData(pageConfig.Title, url)
        .then((data: any) => {
          ...
          if (data.length === 0) {
            setLoadingText(`There are no savings for the ${year} period`);
          } else {
            setLoadingText("");
          }
        })
