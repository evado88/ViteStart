...
const AdminSocialFunds = () => {
  ...
  const pageConfig = new PageConfig(
    "Administration - Approved Social Funds",
    `transactions/type/${Assist.TRANSACTION_SOCIAL_FUND}/status/${Assist.STATUS_APPROVED}`,
    "",
    "My Social Funds",
    "",
    [Assist.ROLE_ADMIN],
  ...
  const loadData = (year: number) => {
    ...
    const url = `transactions/type/${Assist.TRANSACTION_SOCIAL_FUND}/status/${Assist.STATUS_APPROVED}/year/${year}`;
    ...
    setTimeout(() => {
      Assist.loadData(pageConfig.Title, url)
        .then((data: any) => {
          ...
          if (data.length === 0) {
            setLoadingText(`There are no social funds for the ${year} period`);
          } else {
            setLoadingText("");
          }
        })
...
export default AdminSocialFunds;
